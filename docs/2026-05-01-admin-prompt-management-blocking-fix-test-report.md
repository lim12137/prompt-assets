...
3. `$env:PLAYWRIGHT_WEB_DIST='.next-e2e-admin-prompts-race-red'; $env:PLAYWRIGHT_WEB_PORT='36121'; pnpm exec playwright test tests/e2e/admin/prompts-management.spec.ts --grep "旧请求回填" --reporter=line`
   - 结果：失败
   - 摘要：
     - 归档后立即切到“仅看已归档”时，延迟返回的“全部状态”旧请求会把 `gamma-prompt` 回填到当前视图
...
3. `$env:PLAYWRIGHT_WEB_DIST='.next-e2e-admin-prompts-fix'; $env:PLAYWRIGHT_WEB_PORT='36120'; pnpm exec playwright test tests/e2e/admin/prompts-management.spec.ts --reporter=line`
   - 结果：4/4 通过
4. `$env:PLAYWRIGHT_WEB_DIST='.next-e2e-admin-prompts-race-green'; $env:PLAYWRIGHT_WEB_PORT='36122'; pnpm exec playwright test tests/e2e/admin/prompts-management.spec.ts --grep "旧请求回填" --reporter=line`
   - 结果：1/1 通过
5. `$env:PLAYWRIGHT_WEB_DIST='.next-e2e-admin-prompts-race-green'; $env:PLAYWRIGHT_WEB_PORT='36122'; pnpm exec playwright test tests/e2e/admin/prompts-management.spec.ts --reporter=line`
   - 结果：5/5 通过
...
- 管理列表请求增加“最新请求序号”保护，状态操作后的旧筛选请求即使晚到也不会覆盖当前视图
